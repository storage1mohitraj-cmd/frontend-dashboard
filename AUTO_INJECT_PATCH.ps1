...
while ($true) {
    ...
    if ($connection) {
        ...
        scp -i $key -o StrictHostKeyChecking=no "f:\Whiteout Survival Bot\DISCORD BOT\cogs\gift_captchasolver.py" opc@140.245.192.181:"/home/opc/app/bot/DISCORD BOT/cogs/gift_captchasolver.py"
        
        # Restart the daemon safely
        ssh -i $key -o StrictHostKeyChecking=no opc@140.245.192.181 "pm2 start discordbot"
