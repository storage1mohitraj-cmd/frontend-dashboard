...
def format_furnace_level(furnace_lv):
    """
    Unified furnace level formatting matching game logic and user requirements.
    ...
    # Specific requested mapping for Tier 1 & 2
    if val == 35: return "1"
    if 36 <= val <= 39: return f"1-{val - 35}"
    if val == 40: return "2"
    
    # Tier 2 specific mapping (41-43 as 2-1, 44 as 2-2)
    if 41 <= val <= 43: return "2-1"
    if val == 44: return "2-2"
    if val == 45: return "3"

    # Default pattern for 46+ (5 levels per tier)
    # 46-49 = 3-1 to 3-4, 50 = 4, etc.
    relative = val - 45
    tier = (relative // 5) + 3
    stage = relative % 5
