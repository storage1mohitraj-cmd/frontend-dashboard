...
# Install dependencies first (Disabled for debug)
# if not ensure_dependencies_installed():
#     print("[ERROR] Failed to install dependencies")
#     sys.exit(1)
print("[DEBUG] Skipping ensure_dependencies_installed")
...
class PollinateButtonView(discord.ui.View):
    def __init__(self) -> None:
        super().__init__(title="Edit Image", timeout=None)

    @discord.ui.button(label="Regenerate", style=discord.ButtonStyle.secondary, custom_id="regenerate-button")
    async def regenerate(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            await interaction.response.defer()
    ...
    async def edit(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            if not interaction.message.embeds:
    ...
    async def delete(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            author_id = None
            try:
    ...
    async def bookmark(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            if not interaction.message.embeds:
                await interaction.response.send_message("Original embed not found.", ephemeral=True)
                return
...
async def run_initial_async_setup():
    """Run all async initializations that were previously in setup_hook"""
...
# @bot.tree.error
# async def on_app_command_error(interaction: discord.ApplicationContext, error: discord.ApplicationCommandError):
...
@bot.slash_command(name="dice", description="Roll a six-sided dice")
async def dice(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="ask", description="Ask Molly anything about Whiteout Survival or any topic")
async def ask_command(interaction: discord.ApplicationContext, question: str):
    ...
    try:
        ...
        try:
            ...
            if is_game_question and not player_id:
                ...
                class QuickPlayerIDModal(discord.ui.Modal):
                    ...
                    def __init__(self, original_question: str, user_id: str, user_name: str):
                        super().__init__(title="🎮 Setup Your Player ID")
                        self.original_question = original_question
                        self.user_id = user_id
                        self.user_name = user_name

                    async def on_submit(self, modal_interaction: discord.ApplicationContext):
                ...
                class PlayerIDButton(discord.ui.View):
                    ...
                    async def enter_id(self, btn_interaction: discord.ApplicationContext, button: discord.ui.Button):
...
class BirthdayModal(discord.ui.Modal):
    ...
    def __init__(self, target_user: Optional[discord.User] = None):
        super().__init__(title="Add / Update Birthday")
...
class BirthdayView(discord.ui.View):
    ...
    async def add_update(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def remove_entry(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
...
@bot.slash_command(name="birthday", description="Manage your birthday entry (day & month)")
async def birthday(interaction: discord.ApplicationContext):
...
# the @discord.slash_command decorator in `alliance.py`. Removing the
# local wrapper avoids duplicate registrations where both the cog and
# a wrapper attempt to register `/settings`.


@bot.slash_command(name="debug_list_commands", description="(Admin) List registered app commands and their scopes")
@discord.default_permissions(administrator=True)
async def debug_list_commands_wrapper(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="debug_cogs", description="(Admin) List loaded cogs and extensions")
@discord.default_permissions(administrator=True)
async def debug_cogs_wrapper(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="giftcode", description="Show active gift codes")
async def giftcode(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="refresh", description="Clears cached alliance data and reloads from Google Sheets.")
@discord.default_permissions(administrator=True)  # Only server administrators can use this
async def refresh(interaction: discord.ApplicationContext):
...
async def time_autocomplete(interaction: discord.ApplicationContext, current: str):
    ...
    if q and q[0].isdigit():
        try:
            ...
            choices.append(discord.OptionChoice(name=f"in {num}m — in {num} minutes", value=f"in {num}m"))
            choices.append(discord.OptionChoice(name=f"in {num}h — in {num} hours", value=f"in {num}h"))
        except Exception:
            pass

    # quick starts
    if q.startswith("t"):
        choices.append(discord.OptionChoice(name="today 6pm — Today at 6:00 PM", value="today 6pm"))
        choices.append(discord.OptionChoice(name="tomorrow 9am — Tomorrow at 9:00 AM", value="tomorrow 9am"))
    ...
    if q.startswith("on"):
        ...
        choices.append(discord.OptionChoice(name=f"on {date_str} at 3pm — Specific date", value=f"on {date_str} at 3pm"))
        choices.append(discord.OptionChoice(name="on December 25 at 9am — Christmas example", value="on December 25 at 9am"))

    # date-like heuristics
    if q and any(c.isdigit() for c in q) and ('-' in q or '/' in q or ':' in q):
        choices.append(discord.OptionChoice(name="2025-11-05 18:00 — Exact date/time", value="2025-11-05 18:00"))
    ...
    try:
        if q:
            ...
            if parsed_dt:
                ...
                choices.insert(0, discord.OptionChoice(name=f"{current} → {preview} ({user_tz.upper()})", value=current))
    except Exception:
    ...
    for val, desc in templates:
        ...
        if q == "" or val.startswith(q) or q in val or q in desc.lower():
            choices.append(discord.OptionChoice(name=f"{val} — {desc}", value=val))

    return choices[:25]


@bot.slash_command(name="storage_status", description="Show which reminder storage is active and a sample count")
async def storage_status(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="reminder", description="Set a reminder with time and message")
async def reminder(interaction: discord.ApplicationContext, time: str, message: str, channel: discord.TextChannel, body: str = None,
...
@bot.slash_command(name="reminderdashboard", description="Open interactive reminder dashboard (list/delete/set timezone)")
async def reminderdashboard(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="giftcodesettings", description="Open interactive gift code settings dashboard for this server")
@discord.default_permissions(administrator=True)
async def giftcodesettings(interaction: discord.ApplicationContext):
    await interaction.response.defer()
    await animator.show_loading(interaction)
    try:
...
@bot.slash_command(name="imagine", description="Generate AI Images (Pollinations compatibility)")
async def imagine(
...
@bot.slash_command(name="serverstats", description="Show detailed server statistics")
async def serverstats(interaction: discord.ApplicationContext):
...
@bot.slash_command(name="mostactive", description="Show the top 3 most active users and activity graph based on messages in the current month")
async def mostactive(interaction: discord.ApplicationContext):
    guild = interaction.guild
    if not guild:
        await interaction.response.send_message("This command can only be used in a server.", ephemeral=True)
        return
...
@bot.slash_command(name="help", description="Show information about available commands")
...
class DiceBattleView(discord.ui.View):
    ...
    async def _handle_roll(self, interaction: discord.ApplicationContext, player: discord.Member, button: discord.ui.Button):
    ...
    async def roll_challenger(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
    ...
    async def roll_opponent(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        await self._handle_roll(interaction, self.opponent, button)


@bot.slash_command(name="dicebattle", description="Challenge someone to a dice battle")
async def dicebattle(interaction: discord.ApplicationContext, opponent: discord.Member):
...
@bot.slash_command(name="register_view", description="Register a persistent view for an existing message (admin)")
@discord.default_permissions(administrator=True)
async def register_view(interaction: discord.ApplicationContext, channel: discord.TextChannel, message_id: str):
    ...
    try:
        ...
        class ViewTypeSelect(discord.ui.Select):
            ...
            async def callback(self, select_interaction: discord.ApplicationContext):
                try:
...
if __name__ == "__main__":
    ...
    print("Skipping setup_dependencies for debug")
    try:
        startup_cleanup()
    except Exception:
        pass
