...
class Alliance(commands.Cog):
    ...
    async def view_alliances(self, interaction: discord.ApplicationContext):
        try:
    ...
    async def alliance_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        self.c.execute("SELECT alliance_id, name FROM alliance_list")
        alliances = self.c.fetchall()
        return [
            discord.OptionChoice(name=f"{name} (ID: {alliance_id})", value=str(alliance_id))
            for alliance_id, name in alliances if current.lower() in name.lower()
        ][:25]

    async def show_main_menu(self, interaction: discord.ApplicationContext):
        """Programmatic access to settings menu"""
        await self.settings.callback(self, interaction)

    @discord.slash_command(name="settings", description="Open settings menu.")
    @command_animation
    async def settings(self, interaction: discord.ApplicationContext):
        try:
            if interaction.guild is not None: # Check bot permissions only if in a guild
                perm_check = interaction.guild.get_member(interaction.bot.user.id)
        ...
        if interaction.type == discord.InteractionType.component:
            ...
            try:
                if custom_id == "alliance_operations":
                    ...
                    async def alliance_check_callback(select_interaction: discord.ApplicationContext):
                        try:
    ...
    async def add_alliance(self, interaction: discord.ApplicationContext):
        if interaction.guild is None:
        ...
        try:
            ...
            async def channel_select_callback(select_interaction: discord.ApplicationContext):
                try:
    ...
    async def edit_alliance(self, interaction: discord.ApplicationContext):
        self.c.execute("""
        ...
        class PaginatedAllianceView(discord.ui.View):
            ...
            async def previous_callback(self, interaction: discord.ApplicationContext):
            ...
            async def next_callback(self, interaction: discord.ApplicationContext):
                self.current_page = (self.current_page + 1) % len(self.pages)
                self.update_view()
    ...
    async def delete_alliance(self, interaction: discord.ApplicationContext):
        try:
    ...
    async def alliance_delete_callback(self, interaction: discord.ApplicationContext):
        try:
    ...
    async def handle_button_interaction(self, interaction: discord.ApplicationContext):
    ...
    async def bot_operations_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            bot_ops_cog = interaction.client.get_cog("BotOperations")
            if bot_ops_cog:
                await bot_ops_cog.show_bot_operations_menu(interaction)
    ...
    @discord.slash_command(name="alliancemonitor", description="Alliance monitoring dashboard with quick access to all monitoring features")
    @command_animation
    async def alliance_monitor(self, interaction: discord.ApplicationContext):
    ...
    @discord.slash_command(name="allianceactivity", description="Show player growth based on furnace changes (Last 7 Days)")
    @command_animation
    async def alliance_activity(self, interaction: discord.ApplicationContext):
        ...
        try:
            ...
            class PaginatedActivityView(discord.ui.View):
                ...
                async def page_counter(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
                    pass
...
class AllianceMonitorView(discord.ui.View):
    ...
    async def set_log_channel_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        ...
        try:
            ...
            async def channel_callback(select_interaction: discord.ApplicationContext):
    ...
    async def view_status_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
    ...
    async def toggle_monitoring_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
...
class MemberOperationsView(discord.ui.View):
    ...
    async def add_member_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def remove_member_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            alliances = await self.get_admin_alliances(interaction.user.id, interaction.guild.id)
            if not alliances:
    ...
    async def view_members_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            alliances = await self.get_admin_alliances(interaction.user.id, interaction.guild.id)
            if not alliances:
