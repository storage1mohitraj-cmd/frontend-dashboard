...
class GiftCodeAPI:
    def __init__(self, bot):
        ...
        coro = self.start_api_check()
        try:
            asyncio.create_task(coro)
        except RuntimeError:
            # Task will be started manually or by an event listener
            coro.close() # Prevent RuntimeWarning
            self.logger.info("[GIFTCODES] No event loop running, API check task will start later")
