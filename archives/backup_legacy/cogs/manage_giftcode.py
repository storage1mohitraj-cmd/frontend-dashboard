...
try:
    ...
    from admin_utils import is_admin, is_global_admin, is_bot_owner, format_furnace_level
except ImportError:
...
class ManageGiftCode(commands.Cog):
    ...
    def format_furnace_level(self, furnace_lv):
        """Format furnace level display matching game logic (delegated to admin_utils)"""
            ...
            while not redemption_successful:
                ...
                try:
                    ...
                    if status in ["SUCCESS", "SAME TYPE EXCHANGE"]:
                        ...
                        break
                    elif status in ["INVALID_CODE", "EXPIRED", "CDK_NOT_FOUND", "TIME_ERROR", "USAGE_LIMIT"]:
                        # Permanent failures - code itself is bad, not worth retrying
                        self.logger.warning(f"❌ Permanent failure for {nickname}: {status} - code is invalid/expired/limit reached")
                        break
                    else:
    ...
    async def process_auto_redeem(self, guild_id, giftcode):
        ...
        try:
            ...
            async def process_member_with_semaphore(idx, fid, nickname, furnace_lv):
                ...
                nonlocal success_count, failed_count, already_redeemed_count, completed_count, stop_all
                ...
                async with semaphore:
                    ...
                    async with progress_lock:
                        ...
                        # If a code-wide permanent failure occurs, stop the whole process
                        if status in ["CDK_NOT_FOUND", "EXPIRED", "TIME_ERROR", "INVALID_CODE", "USAGE_LIMIT"]:
                            if not stop_all:
                                stop_all = True
                                self.logger.warning(f"⚠️ Code-wide permanent failure '{status}' detected. Halting auto-redeem for all remaining members.")
                        ...
                        try:
                            progress_embed = discord.Embed(
                                ...
                                # Add failure sub-counts if any
                                if error_counts:
                                    for err_status, count in sorted(error_counts.items(), key=lambda x: x[1], reverse=True):
                                        progress_embed.description += f"L `{err_status}: {count}`\n"
            ...
            final_embed = discord.Embed(
                ...
                if error_counts:
                    for err_status, count in sorted(error_counts.items(), key=lambda x: x[1], reverse=True):
                        final_embed.description += f"L `{err_status}: {count}`\n"
    ...
    async def on_interaction(self, interaction: discord.ApplicationContext):
        ...
        if custom_id == "giftcode_add_code":
            ...
            class AddCodeModal(discord.ui.Modal):
                ...
                def __init__(self, cog):
                    super().__init__(title="Add Gift Code")
                    self.cog = cog
        ...
        if custom_id == "auto_redeem_add_via_fid":
            ...
            class AddMemberModal(discord.ui.Modal):
                ...
                def __init__(self, cog):
                    super().__init__(title="Add Member to Auto-Redeem")
                    self.cog = cog
        ...
        if custom_id == "auto_redeem_remove_via_fid":
            ...
            class RemoveMemberModal(discord.ui.Modal):
                ...
                def __init__(self, cog):
                    super().__init__(title="Remove Members")
                    self.cog = cog
        ...
        if custom_id == "auto_redeem_bulk_remove":
            ...
            try:
                ...
                class BulkRemoveSelectView(discord.ui.View):
                    ...
                    async def member_select(self, select_interaction: discord.ApplicationContext):
                    ...
                    async def select_all_callback(self, btn_interaction: discord.ApplicationContext):
                    ...
                    async def deselect_all_callback(self, btn_interaction: discord.ApplicationContext):
                    ...
                    async def remove_selected_members(self, remove_interaction: discord.ApplicationContext):
        ...
        if custom_id == "auto_redeem_auto_register":
            ...
            try:
                ...
                self.logger.error("discord.ApplicationContext expired before defer")
                return
            except Exception as e:
            ...
            try:
                ...
                self.logger.error("discord.ApplicationContext expired during auto register")
            except Exception as e:
        ...
        if custom_id == "auto_redeem_import_from_channel":
            ...
            try:
                ...
                class ChannelSelectView(discord.ui.View):
                    ...
                    async def channel_select(self, select_interaction: discord.ApplicationContext):
                        # Import at method level to avoid scope issues
                        from db.mongo_adapters import mongo_enabled, AutoRedeemChannelsAdapter
...
def setup(bot):
    bot.add_cog(ManageGiftCode(bot))
