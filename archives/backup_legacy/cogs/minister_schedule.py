...
class MinisterSchedule(commands.Cog):
    ...
    async def appointment_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        try:
            choices = [
                discord.OptionChoice(name="Construction Day", value="Construction Day"),
                discord.OptionChoice(name="Research Day", value="Research Day"),
                discord.OptionChoice(name="Troops Training Day", value="Troops Training Day")
            ]
            if current:
    ...
    async def fid_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        try:
    ...
    async def registered_fid_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        try:
            # Fetch selected appointment type from interaction
            appointment_type = next(
                (option["value"] for option in interaction.data.get("options", []) if option["name"] == "appointment_type"),
    ...
    async def time_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        try:
            appointment_type = next(
                (option["value"] for option in interaction.data.get("options", []) if option["name"] == "appointment_type"),
                None
            ...
            if current:
                ...
                filtered_choices = [
                    discord.OptionChoice(name=time, value=time)
    ...
    async def choice_autocomplete(self, interaction: discord.ApplicationContext, current: str):
        try:
            choices = [
                discord.OptionChoice(name="Show full minister list", value="all"),
                discord.OptionChoice(name="Show available slots only", value="available only")
            ]
    ...
    async def get_log_guild(self, log_guild: discord.Guild) -> discord.Optional[Guild ]:
    ...
    @discord.slash_command(name='ministerappointment', description='Manage minister appointments.')
    async def ministerappointment(self, interaction: discord.ApplicationContext):
...
def setup(bot):
    bot.add_cog(MinisterSchedule(bot))
