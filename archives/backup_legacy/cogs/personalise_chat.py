...
class PronounSelectView(discord.ui.View):
    ...
    async def pronoun_select(self, interaction: discord.ApplicationContext, select: discord.ui.Select):
...
class TraitSelectView(discord.ui.View):
    ...
    async def trait_select(self, interaction: discord.ApplicationContext, select: discord.ui.Select):
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("❌ This is not your personalization session.", ephemeral=True)
            return
...
class PlayerIDModal(discord.ui.Modal):
    ...
    def __init__(self, user_id: str, pronouns: str, traits: List[str]):
        super().__init__(title="🎮 Step 3: Player Information")
        self.user_id = user_id
        self.pronouns = pronouns
        self.traits = traits
...
class PersonaliseChatCog(commands.Cog):
    ...
    @command_animation
    async def personalisechat(self, interaction: discord.ApplicationContext):
...
def setup(bot: commands.Bot):
    ...
    bot.add_cog(PersonaliseChatCog(bot))
