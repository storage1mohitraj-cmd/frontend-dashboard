...
class StartView(discord.ui.View):
    ...
    async def alliance_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            ...
            class AllianceAuthModal(discord.ui.Modal):
    ...
    async def giftcodes_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            # Import authentication adapters
            from db.mongo_adapters import mongo_enabled, ServerAllianceAdapter, AuthSessionsAdapter
            ...
            # No valid session - show authentication modal
            class GiftCodeAuthModal(discord.ui.Modal):
                password_input = discord.ui.TextInput(
                    label="Enter Access Code",
                    placeholder="••••••••••••",
                    style=discord.InputTextStyle.short,
                ...
                def __init__(self, guild_id: int, guild_name: str, bot_instance):
                    super().__init__(title="🔐 Security Verification")
                    self.guild_id = guild_id
                    self.guild_name = guild_name
                    self.bot = bot_instance
    ...
    async def events_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            # Import event data
            ...
            class EventSelectionView(discord.ui.View):
                ...
                async def event_selected(self, select_interaction: discord.ApplicationContext):
                    try:
    ...
    async def help_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def reminder_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            ...
            if reminder_cmd:
                ...
                class ReminderSetupView(discord.ui.View):
                    ...
                    async def channel_selected(self, select_interaction: discord.ApplicationContext):
                    ...
                    async def preset_selected(self, select_interaction: discord.ApplicationContext):
                    ...
                    async def set_basic_info(self, button_interaction: discord.ApplicationContext, button: discord.ui.Button):
                        ...
                        class BasicInfoModal(discord.ui.Modal):
                            ...
                            def __init__(self, parent_view):
                                super().__init__(title="⏰ Basic Reminder Info")
                    ...
                    async def add_custom_urls(self, button_interaction: discord.ApplicationContext, button: discord.ui.Button):
                        # Modal for custom URLs
                        class CustomURLModal(discord.ui.Modal):
                            ...
                            def __init__(self, parent_view):
                                super().__init__(title="🔗 Custom URLs")
                                self.parent_view = parent_view
                    ...
                    async def create_reminder(self, button_interaction: discord.ApplicationContext, button: discord.ui.Button):
    ...
    async def music_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def autotranslate_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def settings_button(self, button: discord.ui.Button, interaction: discord.Interaction):
        # Handle potential argument swap between py-cord versions
        real_interaction = interaction
        if isinstance(button, (discord.Interaction, discord.ApplicationContext)):
             real_interaction = button
             # real_button = interaction
        elif isinstance(interaction, discord.ui.Button):
             # Arguments are (self, interaction, button) - seemingly correct in signature but types say otherwise?
             pass 

        # If the first argument passed was actually the button (which caused the error), correct it.
        # Function signature above changed to (self, button, interaction) to match observation.
        
        try:
            alliance_cog = self.bot.get_cog("Alliance")
            if alliance_cog and hasattr(alliance_cog, 'settings'):
                # Call the settings command from the Alliance cog
                await alliance_cog.settings.callback(alliance_cog, real_interaction)
            else:
                await real_interaction.response.send_message("Settings command is not available.", ephemeral=True)
        except Exception as e:
            logger.error(f"Error in settings button: {e}")
            try:
                await real_interaction.response.send_message("Failed to open Settings menu.", ephemeral=True)
            except:
                pass

    @discord.ui.button(label="Games", style=discord.ButtonStyle.success, emoji="🎮", custom_id="start_games")
    async def games_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def birthday_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            import cogs.shared_views as sv
    ...
    async def welcome_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def manage_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
...
class AutoTranslateMenuView(discord.ui.View):
    ...
    async def create_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
    ...
    async def edit_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            autotranslate_cog = self.bot.get_cog("AutoTranslate")
    ...
    async def toggle_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            autotranslate_cog = self.bot.get_cog("AutoTranslate")
    ...
    async def delete_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
            autotranslate_cog = self.bot.get_cog("AutoTranslate")
...
class GamesView(discord.ui.View):
    ...
    async def dice_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
        try:
...
class StartMenu(commands.Cog):
    ...
    @discord.slash_command(name="start", description="Show the main menu")
    @command_animation
    async def start(self, interaction: discord.ApplicationContext):
        embed = discord.Embed(
            title="🤖 Bot Main Menu",
        ...
        try:
            ...
            if e.code == 40060: # Interaction already acknowledged
                 # Re-open file as it might be closed/consumed
                 logo_file = discord.File("logo.png", filename="logo.png")
                 await interaction.followup.send(embed=embed, view=view, file=logo_file)
            else:
                 raise
