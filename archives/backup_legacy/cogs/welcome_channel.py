...
class BGImageModal(discord.ui.Modal):
...
class ChannelSelectView(discord.ui.View):
    ...
    async def channel_select(self, interaction: discord.ApplicationContext, select: discord.ui.ChannelSelect):
...
class WelcomeMenuView(discord.ui.View):
    ...
    async def set_channel_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
    ...
    async def bg_image_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
    ...
    async def welcome_status_button(self, interaction: discord.ApplicationContext, button: discord.ui.Button):
...
class WelcomeChannel(commands.Cog):
    ...
    async def create_welcome_image(self, member: discord.Member) -> io.BytesIO:
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to load project font: {e}")
                try:
                    # Fallback to arial
                    ...
                    logger.warning("Failed to load arial, using default font")
            ...
            # Line 2: to Server Name
            server_text = f"to {member.guild.name}"

            # Line 3: Member count
            member_count = member.guild.member_count
            count_text = f"you are the {member_count}th member!"
            ...
            # Draw Welcome line
            draw.text((text_x, current_y), welcome_text, fill=text_color, font=font_medium,
                     stroke_width=stroke_width, stroke_fill=stroke_fill)

            current_y += 50

            # Draw Server Name line
            draw.text((text_x, current_y), server_text, fill=text_color, font=font_large,
                     stroke_width=stroke_width, stroke_fill=stroke_fill)

            current_y += 70

            # Draw Member Count line
            draw.text((text_x, current_y), count_text, fill=text_color, font=font_small,
                     stroke_width=stroke_width, stroke_fill=stroke_fill)
    ...
    async def check_admin_permission(self, interaction: discord.ApplicationContext) -> bool:
    ...
    @discord.slash_command(name="welcome", description="Configure welcome message settings")
    @command_animation
    async def welcome(self, interaction: discord.ApplicationContext):
    ...
    @discord.slash_command(name="removewelcomechannel", description="Remove the welcome channel configuration")
    @command_animation
    async def remove_welcome_channel(self, interaction: discord.ApplicationContext):
