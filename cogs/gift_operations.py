...
class GiftView(discord.ui.View):
    ...
    @discord.ui.button(
        label="Auto Redeem Members",
        style=discord.ButtonStyle.primary,
        custom_id="giftcode_auto_redeem",
        emoji="🤖",
        row=1
    )
    async def auto_redeem_members_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        # Delegate to ManageGiftCode cog
        manage_cog = interaction.client.get_cog('ManageGiftCode')
        if manage_cog:
            await manage_cog.on_interaction(interaction)
        else:
            await interaction.response.send_message("❌ Manage Gift Code module not found.", ephemeral=True)
