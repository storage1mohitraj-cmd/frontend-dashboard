...
class AttendanceSettingsView(discord.ui.View):
    ...
    async def report_type_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            embed = discord.Embed(
                title="≡ƒôè Report Type Settings",
                description=(
                    f"**Current Setting:** {current_setting.title()}\n\n"
                    "**Available Options:**\n"
                    "ΓÇó **Text** - Text-based reports (faster, no requirements)\n"
                    "ΓÇó **Matplotlib** - Visual table reports (requires matplotlib)\n\n"
                    f"**Matplotlib Status:** {'Γ£à Available' if MATPLOTLIB_AVAILABLE else 'Γ¥î Not Available'}\n\n"
    ...
    async def matplotlib_reports_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not MATPLOTLIB_AVAILABLE:
            await interaction.response.send_message(
                "Γ¥î Matplotlib is not available on this system.",
                ephemeral=True
            )
            return
    ...
    async def view_attendance_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            select_embed = discord.Embed(
                title="≡ƒæÇ View Attendance - Alliance Selection",
...
class EventTypeSelectView(discord.ui.View):
    def __init__(self, session_data, cog, alliance_id, alliance_name):
        ...
        # Add back button
        back_button = discord.ui.Button(label="Γ¼à∩╕Å Back", style=discord.ButtonStyle.secondary)
        back_button.callback = self.back_to_sessions
        self.add_item(back_button)
    
    def create_event_type_select(self):
        select = discord.ui.Select(
            placeholder="≡ƒÄ» Select Event Type...",
            options=[
                discord.SelectOption(label="Foundry", value="Foundry", emoji="≡ƒÅ¡"),
                discord.SelectOption(label="Canyon Clash", value="Canyon Clash", emoji="ΓÜö∩╕Å"),
                discord.SelectOption(label="Crazy Joe", value="Crazy Joe", emoji="≡ƒñ¬"),
                discord.SelectOption(label="Bear Trap", value="Bear Trap", emoji="≡ƒÉ╗"),
                discord.SelectOption(label="Castle Battle", value="Castle Battle", emoji="≡ƒÅ░"),
                discord.SelectOption(label="Frostdragon Tyrant", value="Frostdragon Tyrant", emoji="≡ƒÉë"),
                discord.SelectOption(label="Other", value="Other", emoji="≡ƒôï", default=True)
            ]
        )
...
class AllianceSelectView(discord.ui.View):
    ...
    def update_select_menu(self):
        ...
        select = discord.ui.Select(
            placeholder=f"≡ƒÅ░ Select an alliance... (Page {self.page + 1}/{self.max_page + 1})",
            options=[
        ...
        prev_button = next((item for item in self.children if hasattr(item, 'label') and item.label == "ΓùÇ∩╕Å"), None)
        next_button = next((item for item in self.children if hasattr(item, 'label') and item.label == "Γû╢∩╕Å"), None)
    ...
    @discord.ui.button(label="ΓùÇ∩╕Å", style=discord.ButtonStyle.secondary, row=1)
    async def prev_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        self.page = max(0, self.page - 1)
        self.update_select_menu()
        await interaction.response.edit_message(view=self)

    @discord.ui.button(label="Γû╢∩╕Å", style=discord.ButtonStyle.secondary, row=1)
...
class EditEventDetailsView(discord.ui.View):
    def __init__(self, session_id, session_name, current_event_type, current_event_date, parent_view, is_edit=True):
        ...
        self.event_type_select = discord.ui.Select(
            ...
            options=[
                discord.SelectOption(label="Foundry", value="Foundry", emoji="≡ƒÅ¡", default=(current_event_type == "Foundry")),
                discord.SelectOption(label="Canyon Clash", value="Canyon Clash", emoji="ΓÜö∩╕Å", default=(current_event_type == "Canyon Clash")),
                discord.SelectOption(label="Crazy Joe", value="Crazy Joe", emoji="≡ƒñ¬", default=(current_event_type == "Crazy Joe")),
                discord.SelectOption(label="Bear Trap", value="Bear Trap", emoji="≡ƒÉ╗", default=(current_event_type == "Bear Trap")),
                discord.SelectOption(label="Castle Battle", value="Castle Battle", emoji="≡ƒÅ░", default=(current_event_type == "Castle Battle")),
                discord.SelectOption(label="Frostdragon Tyrant", value="Frostdragon Tyrant", emoji="≡ƒÉë", default=(current_event_type == "Frostdragon Tyrant")),
                discord.SelectOption(label="Other", value="Other", emoji="≡ƒôï", default=(current_event_type == "Other" or not current_event_type))
            ],
            row=1
        )
        ...
        if not self.is_edit:
            for item in self.children:
                if hasattr(item, 'label') and item.label == "≡ƒùæ∩╕Å Delete Event":
                    item.disabled = True
    ...
    @discord.ui.button(label="Γ£Å∩╕Å Rename Session", style=discord.ButtonStyle.secondary, row=0)
    ...
    @discord.ui.button(label="≡ƒôà Edit Date", style=discord.ButtonStyle.secondary, row=0)
    ...
    @discord.ui.button(label="≡ƒÆ╛ Save", style=discord.ButtonStyle.primary, row=2)
    async def save_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.response.send_message(
                f"Γ¥î Error updating event details: {str(e)}",
                ephemeral=True
            )
            
    @discord.ui.button(label="Γ¥î Cancel", style=discord.ButtonStyle.danger, row=2)
    async def cancel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.parent_view.update_main_embed(interaction)
    
    @discord.ui.button(label="≡ƒùæ∩╕Å Delete Event", style=discord.ButtonStyle.danger, row=3)
        ...
        try:
            ...
            if self.event_date_input.value.strip():
                try:
                    ...
                    await interaction.response.send_message(
                        f"Γ£à Date updated to: {event_date.strftime('%Y-%m-%d %H:%M')} UTC",
                        ephemeral=True
                    )
                except ValueError:
                    await interaction.response.send_message(
                        "Γ¥î Invalid date format. Please use: YYYY-MM-DD HH:MM",
                        ephemeral=True
                    )
            else:
                await interaction.response.send_message(
...
class RenameSessionModal(discord.ui.Modal, title="Rename Session"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            if not new_name:
                await interaction.response.send_message(
                    "Γ¥î Session name cannot be empty.",
                    ephemeral=True
                )
                return
            ...
            await interaction.response.send_message(
                f"Γ£à Session renamed to: **{new_name}**",
                ephemeral=True
            )
            ...
            await interaction.response.send_message(
                f"Γ¥î Error renaming session: {str(e)}",
                ephemeral=True
            )
...
class ConfirmDeleteView(discord.ui.View):
    ...
    @discord.ui.button(label="≡ƒùæ∩╕Å Delete", style=discord.ButtonStyle.danger)
    async def confirm_delete(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.response.send_message(
                f"Γ¥î Error deleting session: {str(e)}",
                ephemeral=True
            )
    
    @discord.ui.button(label="Γ¥î Cancel", style=discord.ButtonStyle.secondary)
...
class PlayerSelectView(discord.ui.View):
    ...
    def update_clear_button_visibility(self):
        ...
        clear_button = next((item for item in self.children if hasattr(item, 'label') and item.label == "Γ¥î Clear"), None)
        if clear_button:
    ...
    def update_select_menu(self):
        ...
        if self.filter_text:
            if not options:
                ...
                placeholder = f"Γ¥î No results for '{self.filter_text}'"
                ...
                placeholder = f"≡ƒæÑ Filtered: '{self.filter_text}' - {len(self.filtered_players)} results (Page {self.page + 1}/{self.max_page + 1})"
        else:
            placeholder = f"≡ƒæÑ Select a player to mark attendance... (Page {self.page + 1}/{self.max_page + 1})"
    ...
    async def show_player_attendance_options(self, interaction: discord.Interaction, player):
        ...
        embed = discord.Embed(
            title=f"≡ƒôï Mark Attendance - {nickname}",
            description=(
                f"**Player:** {nickname}\n"
                f"**FID:** {fid}\n"
    ...
    @discord.ui.button(label="≡ƒöì Filter", style=discord.ButtonStyle.secondary, row=1)
    ...
    @discord.ui.button(label="Γ¥î Clear", style=discord.ButtonStyle.danger, row=1, disabled=True)
    ...
    @discord.ui.button(label="ΓÜÖ∩╕Å Edit Event", style=discord.ButtonStyle.secondary, row=1)
    ...
    @discord.ui.button(label="≡ƒôè View Summary", style=discord.ButtonStyle.primary, row=2)
    ...
    @discord.ui.button(label="Γ£à Finish Attendance", style=discord.ButtonStyle.success, row=2)
    async def finish_attendance_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.edit_original_response(
                content=f"Γ¥î An error occurred while processing attendance: {str(e)}",
                embed=None,
                view=None
            )

    @discord.ui.button(label="Γ¼à∩╕Å Back", style=discord.ButtonStyle.secondary, row=2)
    ...
    async def update_main_embed(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"≡ƒôï Marking Attendance - {self.alliance_name}",
    ...
    async def show_summary(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if report_cog:
                ...
                await interaction.response.send_message(
                    "Γ¥î Attendance Report module not loaded.",
                    ephemeral=True
                )
        except Exception as e:
            ...
            if not interaction.response.is_done():
                await interaction.response.send_message(
                    f"Γ¥î An error occurred while generating the summary: {str(e)}",
                    ephemeral=True
                )
...
class AttendanceModal(discord.ui.Modal):
    ...
    async def update_main_embed_with_confirmation(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=f"≡ƒôï Marking Attendance - {self.parent_view.alliance_name}",
            description=(
                f"**Session:** {self.parent_view.session_name}\n"
                f"**Progress:** {marked_count}/{total_count} players marked\n"
                f"**Current Page:** {self.parent_view.page + 1}/{self.parent_view.max_page + 1}\n\n"
                f"Γ£à **{self.nickname}** marked as **{status_display}** with **{points:,} points**\n\n"
                "Select a player from the dropdown to mark their attendance.\n"
                "Use the buttons below to navigate, view summary, or finish."
            ),
            color=discord.Color.green()
...
class PlayerAttendanceView(discord.ui.View):
    ...
    @discord.ui.button(label="Γ¼à∩╕Å Back to List", style=discord.ButtonStyle.secondary, custom_id="back_to_list")
    async def back_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.parent_view.update_main_embed(interaction)
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"**Progress:** {marked_count}/{total_count} players marked\n"
                    f"**Current Page:** {self.parent_view.page + 1}/{self.parent_view.max_page + 1}\n\n"
                    f"Γ£à **{self.nickname}** marked as **{status_display}**\n\n"
                    "Select a player from the dropdown to mark their attendance.\n"
                    "Use the buttons below to navigate, view summary, or finish."
                ),
                color=discord.Color.green()
...
class Attendance(commands.Cog):
    ...
    def _get_status_emoji(self, status):
        ...
        return {"present": "Γ£à", "absent": "Γ¥î", "not_recorded": "ΓÜ¬"}.get(status, "Γ¥ô")
    ...
    def _format_last_attendance(self, last_attendance):
        ...
        replacements = [
            ("present", "Γ£à"), ("Present", "Γ£à"),
            ("absent", "Γ¥î"), ("Absent", "Γ¥î"),
            ("not_recorded", "ΓÜ¬"), ("Not Recorded", "ΓÜ¬"), ("not recorded", "ΓÜ¬")
        ]
    ...
    async def show_attendance_menu(self, interaction: discord.Interaction):
        ...
        if interaction.guild is None:
            await interaction.response.send_message(
                "Γ¥î This command can only be used in a server, not in DMs.",
                ephemeral=True
            )
            return
            
        embed = discord.Embed(
            title="≡ƒôï Attendance System",
            description=(
                "Please select an operation:\n\n"
                "**Available Operations**\n"
                "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
                "≡ƒôï **Mark Attendance**\n"
                "Γöö Create or modify attendance records\n\n"
                "≡ƒæÇ **View Attendance**\n"
                "Γöö View attendance records and export reports\n\n"
                "ΓÜÖ∩╕Å **Settings**\n"
                "Γöö Configure attendance preferences\n"
                "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü"
            ),
            color=discord.Color.blue()
        )
    ...
    async def show_alliance_selection_for_marking(self, interaction: discord.Interaction):
        ...
        try:
            ...
            select_embed = discord.Embed(
                title="≡ƒôï Attendance - Alliance Selection",
                description=(
                    "Please select an alliance to mark attendance:\n\n"
                    "**Permission Details**\n"
                    "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
                    f"≡ƒæñ **Access Level:** `{'Global Admin' if is_initial == 1 else 'Server Admin'}`\n"
                    f"≡ƒöì **Access Type:** `{'All Alliances' if is_initial == 1 else 'Server + Special Access'}`\n"
                    f"≡ƒôè **Available Alliances:** `{len(alliances)}`\n"
                    "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü"
                ),
                color=discord.Color.blue()
            )
    ...
    async def show_session_selection_for_marking(self, interaction: discord.Interaction, alliance_id: int):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"≡ƒôï Mark Attendance - {alliance_name}",
    ...
    async def show_attendance_marking(self, interaction: discord.Interaction, alliance_id: int, alliance_name: str, session_name: str, session_id: int = None, is_edit: bool = False, event_type: str = "Other", event_date: datetime = None):
        ...
        try:
            ...
            event_icon = EVENT_TYPE_ICONS.get(event_type, "≡ƒôï")
            embed = discord.Embed(
                title=f"≡ƒôï Mark Attendance - {alliance_name}",
                description=(
                    ...
                    f"**Status:** Γ£à Present: {present_count} | Γ¥î Absent: {absent_count} | Γ¥ô Not Recorded: {not_recorded_count}\n\n"
    ...
    async def process_attendance_results(self, interaction: discord.Interaction, selected_players: dict, alliance_name: str, session_name: str, use_defer: bool = True, session_id: int = None, is_edit: bool = False, event_type: str = "Other", event_date: datetime = None, alliance_id: int = None):
        ...
        try:
            ...
            success_embed = discord.Embed(
                title="Γ£à Attendance Saved Successfully",
                description=(
                    f"**Session:** {session_name}\n"
                    f"**Alliance:** {alliance_name}\n"
                    ...
                    f"ΓÜ¬ Not Recorded: {actual_not_recorded}\n"
    ...
    def __init__(self, sessions, alliance_id, cog, is_viewing=False):
        ...
        if sessions:
            ...
            for session in sessions[:25]:  # Discord limit
                event_icon = EVENT_TYPE_ICONS.get(session.get('event_type', 'Other'), '≡ƒôï')
                options.append(discord.SelectOption(
            ...
            select = discord.ui.Select(
                placeholder="≡ƒôï Select a session...",
                options=options
            )
    ...
    async def on_select(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if selected_session:
                ...
                await interaction.edit_original_response(
                    content="Γ¥î Session not found.",
                    embed=None,
                    view=None
                )
        except Exception as e:
            await interaction.edit_original_response(
                content="Γ¥î An error occurred while loading the session.",
                embed=None,
                view=None
            )
...
async def setup(bot):
    try:
        ...
        print(f"Γ¥î Failed to load Attendance cog: {e}")
