...
class ExportFormatSelectView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(label="CSV", value="csv", description="Comma-separated values", emoji="≡ƒôä"),
            discord.SelectOption(label="TSV", value="tsv", description="Tab-separated values", emoji="≡ƒôï"),
            discord.SelectOption(label="HTML", value="html", description="Web page format", emoji="≡ƒîÉ")
        ]
    )
...
"""
        ...
        try:
            ...
            if format_type == "csv":
                ...
                await interaction.followup.send(
                    "Γ¥î Invalid export format selected.",
                    ephemeral=True
                )
                return
            ...
            try:
                await interaction.user.send(
                    f"≡ƒôè **Attendance Report Export**\n"
                ...
                await interaction.followup.send(
                    "Γ£à Attendance report sent to your DMs!",
                    ephemeral=True
                )
            except discord.Forbidden:
                await interaction.followup.send(
                    "Γ¥î Could not send DM. Please enable DMs from server members and try again.",
                    ephemeral=True
                )
            except discord.HTTPException as e:
                if "Maximum message size" in str(e):
                    await interaction.followup.send(
                        "Γ¥î Report too large to send via Discord (8MB limit). Please try exporting fewer records.",
                        ephemeral=True
                    )
                else:
                    await interaction.followup.send(
                        f"Γ¥î An error occurred while sending the report: {str(e)}",
                        ephemeral=True
                    )
            ...
            await interaction.followup.send(
                "Γ¥î An error occurred while generating the export.",
                ephemeral=True
            )
    ...
    async def show_attendance_report(self, interaction: discord.Interaction, alliance_id: int, session_name: str, 
        ...
        try:            
            ...
            await interaction.edit_original_response(
                content="Γ¥î An error occurred while generating attendance report.",
                embed=None,
                view=None
            )
    ...
    async def show_matplotlib_report(self, interaction: discord.Interaction, alliance_id: int, session_name: str,
        ...
        try:
            ...
            if is_preview and selected_players:
                ...
                if not records:
                    await interaction.response.edit_message(
                        content=f"Γ¥î No attendance has been marked yet.",
                        embed=None,
                        view=None
                    )
                    return
                ...
                if not records:
                    await interaction.response.edit_message(
                        content=f"Γ¥î No attendance records found for session '{session_name}' in {alliance_name}.",
                        embed=None,
                        view=None
                    )
                    return
            ...
            embed_title = f"≡ƒôè Attendance Report - {alliance_name}"
    ...
    async def show_text_report(self, interaction: discord.Interaction, alliance_id: int, session_name: str,
        ...
        try:
            ...
            # Summary section
            report_sections.append("≡ƒôè **SUMMARY**")
            ...
            report_sections.append("≡ƒæÑ **PLAYER DETAILS**")
            report_sections.append("ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü")
            
            # Sort: Present (by points desc) ΓåÆ Absent
            def sort_key(record):
            ...
            embed = discord.Embed(
                title=f"≡ƒôè Attendance Report - {alliance_name}",
            ...
            error_content = "Γ¥î An error occurred while generating attendance report."
            if interaction.response.is_done():
    ...
    async def show_session_selection(self, interaction: discord.Interaction, alliance_id: int):
        ...
        try:
            ...
            if not sessions:
                ...
                embed = discord.Embed(
                    title=f"≡ƒôï Attendance Sessions - {alliance_name}",
                    description=f"Γ¥î **No attendance sessions found for {alliance_name}.**\n\nTo create attendance records, use the 'Mark Attendance' option from the main menu.",
                ...
                back_button = discord.ui.Button(
                    label="Γ¼à∩╕Å Back to Alliance Selection",
                    style=discord.ButtonStyle.secondary
                )
            ...
            if interaction.response.is_done():
                await interaction.edit_original_response(
                    content="Γ¥î An error occurred while loading sessions.",
                    embed=None,
                    view=None
                )
            else:
                await interaction.response.send_message(
                    "Γ¥î An error occurred while loading sessions.",
                    ephemeral=True
                )
...
async def setup(bot):
    try:
        ...
        print(f"Γ¥î Failed to load AttendanceReport cog: {e}")
