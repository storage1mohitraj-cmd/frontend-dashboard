...
class EmbedDataView(discord.ui.View):
    ...
    @discord.ui.button(label="Edit Notification settings", style=discord.ButtonStyle.primary, emoji="ΓÜÖ∩╕Å")
    async def notification_setting(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not result:
            await interaction.followup.send("Γ¥î Notification not found in database.", ephemeral=True)
            return
        ...
        embed = discord.Embed(
            ...
            description=(
                f"**≡ƒôà Next Notification date:** {next_notification_date}\n"
                f"**ΓÅ░ Time:** {hours:02d}:{minutes:02d} ({timezone})\n"
                f"**≡ƒôó Channel:** <#{channel_id}>\n"
                f"**≡ƒô¥ Description:** {description}\n\n"
                f"**ΓÜÖ∩╕Å Notification Type**\n{formatted_type}\n\n"
                f"**≡ƒæÑ Mention:** {formatted_mention}\n"
                f"**≡ƒöä Repeat:** {formatted_repeat}\n"
            ),
...
class PlainEditorView(discord.ui.View):
    ...
    async def update_embed(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                f"**≡ƒôà Next Notification date:** {next_notification_date}\n"
                f"**ΓÅ░ Time:** {self.hours:02d}:{self.minutes:02d} ({self.timezone})\n"
                f"**≡ƒôó Channel:** <#{self.channel_id}>\n"
                f"**≡ƒô¥ Description:** {self.description}\n\n"
                f"**ΓÜÖ∩╕Å Notification Type**\n{formatted_type}\n\n"
                f"**≡ƒæÑ Mention:** {formatted_mention}\n"
                f"**≡ƒöä Repeat:** {formatted_repeat}\n"
            ),
            ...
            class DescriptionModal(discord.ui.Modal, title="Edit Description"):
                ...
                async def on_submit(self, modal_interaction: discord.Interaction):
                    ...
                    try:
                        ...
                        await modal_interaction.followup.send("Γ¥î An error occurred!", ephemeral=True)
            ...
            async def on_submit(self, modal_interaction: discord.Interaction):
                ...
                try:
                    ...
                    if not hasattr(self.parent_view, "next_notification"):
                        await modal_interaction.followup.send("Γ¥î Error: `next_notification` is missing!",
                                                              ephemeral=True)
                        return
                    ...
                    if new_date:
                        try:
                            ...
                            await modal_interaction.followup.send("Γ¥î Invalid date format! Use DD/MM/YYYY.",
                                                                  ephemeral=True)
                            return
                    ...
                    await modal_interaction.followup.send("Γ¥î Invalid input! Please enter numbers only.", ephemeral=True)
                except Exception as e:
    ...
    async def edit_mention(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def mention_callback(mention_interaction: discord.Interaction, mention_type: str):
            ...
            if mention_type == "everyone":
                ...
                role_select = discord.ui.RoleSelect(
                    placeholder="≡ƒöì Search and select who to mention...",
                    min_values=1,
                    max_values=1
                )
        ...
        for label, mention_type in [("≡ƒôó everyone", "everyone"), ("≡ƒæÑ Select Role", "role"),
                                    ("≡ƒæñ Select Member", "member"), ("≡ƒöò No Mention", "none")]:
    ...
    async def edit_notification_ping(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        for label, value in options:
            async def callback(interaction: discord.Interaction, value=value):
                ...
                if value == 6:  # Custom Times button pressed
                    class CustomTimeModal(discord.ui.Modal, title="Enter Custom Notification Times"):
                        ...
                        async def on_submit(self, modal_interaction: discord.Interaction):
                            ...
                            # Γ£à Validate format (only numbers and dashes allowed)
                            if not all(c.isdigit() or c == '-' for c in new_times):
                                await modal_interaction.response.send_message(
                                    "Γ¥î Invalid format! Use numbers separated by '-'.", ephemeral=True)
                                return

                            # Γ£à Check if description contains "CUSTOM_TIMES:"
                            ...
                            # Γ£à Update notification and embed
                            await self.parent_view.cog.update_notification(self.parent_view)
                            await self.parent_view.update_embed(modal_interaction)
                            await modal_interaction.response.defer()
                ...
                # Γ£à Remove "CUSTOM_TIMES" if any other option is chosen
        ...
        embed = discord.Embed(
            title="ΓÅ░ Select Notification Type",
            description=(
                "Choose when to send notifications:\n\n"
                "**30m, 10m, 5m & Time**\n"
                "ΓÇó 30 minutes before\n"
                "ΓÇó 10 minutes before\n"
                "ΓÇó 5 minutes before\n"
                "ΓÇó When time's up\n\n"
                "**10m, 5m & Time**\n"
                "ΓÇó 10 minutes before\n"
                "ΓÇó 5 minutes before\n"
                ...
                "ΓÇó When time's up\n\n"
                "**Only 5m**\n"
                "ΓÇó Only 5 minutes before\n\n"
                "**Only Time**\n"
                "ΓÇó Only when time's up\n\n"
                "**Custom Times**\n"
                "ΓÇó Set your own notification times"
            ),
            color=discord.Color.blue()
        )
...
class NotificationEditor(commands.Cog):
    ...
    async def start_edit_process(self, interaction: discord.Interaction, notification_id: int,
        ...
        if not result:
            await interaction.response.send_message("Γ¥î Notification ID not found.", ephemeral=True)
            return
