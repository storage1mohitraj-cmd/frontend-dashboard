...
class Control(commands.Cog):
    ...
    async def check_agslist(self, channel, alliance_id, interaction=None):
        ...
        embed = discord.Embed(
            title=f"≡ƒÅ░ {alliance_name} Alliance Control",
            description="≡ƒöì Checking for changes in member status...",
            color=discord.Color.blue()
        )
        embed.add_field(
            name="≡ƒôè Status",
            value=f"ΓÅ│ Control started at {start_time.strftime('%Y-%m-%d %H:%M:%S')}",
            inline=False
        )
        embed.add_field(
            name="≡ƒôê Progress",
            value=f"Γ£¿ Members checked: {checked_users}/{total_users}",
            inline=False
        )
        ...
        while i < total_users:
            ...
            for fid, old_nickname, old_furnace_lv, old_stove_lv_content, old_kid in batch_users:
                ...
                if data == 429:
                    ...
                    embed.description = f"ΓÜá∩╕Å API Rate Limit! Waiting {wait_time:.1f} seconds...\n≡ƒôè Progress: {checked_users}/{total_users} members"
                    ...
                    embed.description = "≡ƒöì Checking for changes in member status..."
                ...
                if isinstance(data, dict):
                    if 'error' in data:
                        ...
                        if error_msg == 'not_found':
                            ...
                            if removed:
                                check_fail_list.append(f"Γ¥î `{fid}` ({old_nickname}) - Player not found (Auto-removed)")
                            else:
                                check_fail_list.append(f"Γ¥î `{fid}` - Player not found (Failed to remove)")
                        else:
                            # For other errors, just report without removing
                            check_fail_list.append(f"Γ¥î `{fid}` - {error_msg}")
                        ...
                        async with self.db_lock:
                            ...
                            if old_kid != new_kid:
                                kid_changes.append(f"≡ƒæñ **{old_nickname}** has transferred to a new state\n≡ƒöä Old State: `{old_kid}`\n≡ƒåò New State: `{new_kid}`")
                            ...
                            if new_furnace_lv != old_furnace_lv:
                                ...
                                self.conn_users.commit()
                                furnace_changes.append(f"≡ƒæñ **{old_nickname}**\n≡ƒöÑ `{old_furnace_display}` Γ₧í∩╕Å `{new_furnace_display}`")
                            ...
                            if new_nickname.lower() != old_nickname.lower().strip():
                                ...
                                self.conn_users.commit()
                                nickname_changes.append(f"≡ƒô¥ `{old_nickname}` Γ₧í∩╕Å `{new_nickname}`")
        ...
        if furnace_changes or nickname_changes or kid_changes or check_fail_list:
            if furnace_changes:
                await self.send_embed(
                    channel=channel,
                    title=f"≡ƒöÑ **{alliance_name}** Furnace Level Changes",
                    description=safe_list(furnace_changes),
                    color=discord.Color.orange(),
                    footer=f"≡ƒôè Total Changes: {len(furnace_changes)}"
                )

            if nickname_changes:
                await self.send_embed(
                    channel=channel,
                    title=f"≡ƒô¥ **{alliance_name}** Nickname Changes",
                    description=safe_list(nickname_changes),
                    color=discord.Color.blue(),
                    footer=f"≡ƒôè Total Changes: {len(nickname_changes)}"
                )

            if kid_changes:
                await self.send_embed(
                    channel=channel,
                    title=f"≡ƒîì **{alliance_name}** State Transfer Notifications",
                    description=safe_list(kid_changes),
                    color=discord.Color.green(),
                    footer=f"≡ƒôè Total Changes: {len(kid_changes)}"
                )
            ...
            if check_fail_list:
                ...
                footer_text = f"≡ƒôè Total Issues: {len(check_fail_list)}"
                if auto_removed_count > 0:
                    footer_text += f" | ≡ƒùæ∩╕Å Auto-removed: {auto_removed_count}"
                
                await self.send_embed(
                    channel=channel,
                    title=f"Γ¥î **{alliance_name}** Invalid Members Detected",
            ...
            embed.set_field_at(
                0,
                name="≡ƒôè Final Status",
                value=f"Γ£à Control completed with changes\nΓÅ░ {end_time.strftime('%Y-%m-%d %H:%M:%S')}",
                inline=False
            )
            embed.add_field(
                name="ΓÅ▒∩╕Å Duration",
            ...
            embed.add_field(
                name="≡ƒôê Total Changes",
                value=f"≡ƒöä {len(furnace_changes) + len(nickname_changes) + len(kid_changes)} changes detected" + (f"\n≡ƒùæ∩╕Å {sum(1 for item in check_fail_list if 'Auto-removed' in item)} invalid FIDs removed" if any('Auto-removed' in item for item in check_fail_list) else "") + (f"\nΓ¥î {sum(1 for item in check_fail_list if 'Auto-removed' not in item)} check failures" if any('Auto-removed' not in item for item in check_fail_list) else ""),
                inline=True
            )
        else:
            embed.color = discord.Color.green()
            embed.set_field_at(
                0,
                name="≡ƒôè Final Status",
                value=f"Γ£à Control completed successfully\nΓÅ░ {end_time.strftime('%Y-%m-%d %H:%M:%S')}\n≡ƒô¥ No changes detected",
                inline=False
            )
            embed.add_field(
                name="ΓÅ▒∩╕Å Duration",
