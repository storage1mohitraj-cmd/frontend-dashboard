    ...
    def __init__(self, bot):
        ...
        if self.mongo_enabled:
            ...
            try:
                ...
                self.logger.info("[GIFTCODES] Γ£à MongoDB enabled - Using mongo adapters for all operations")
        ...
        if not self.mongo_enabled:
            ...
            self.logger = logging.getLogger("gift_operationsapi")
            self.logger.info("[GIFTCODES] ΓÜá∩╕Å MongoDB not configured - Falling back to SQLite")
        ...
        if self.monitor_channel_id:
            self.logger.info(f"[GIFTCODES] ≡ƒô║ Monitoring channel configured: {self.monitor_channel_id}")
                ...
                try:
                    async with session.get(self.api_url, headers=headers) as response:
                        ...
                        try:
                                        ...
                                        try:
                                            ...
                                            if admin_ids:
                                                admin_embed = discord.Embed(
                                                    title="≡ƒÄü New Gift Code Found!",
                                                    description=(
                                                        f"**Gift Code Details**\n"
                                                        f"ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
                                                        f"≡ƒÄü **Code:** `{code}`\n"
                                                        f"≡ƒôà **Date:** `{formatted_date}`\n"
                                                        f"≡ƒô¥ **Status:** `Retrieved from Bot API`\n"
                                                        f"ΓÅ░ **Time:** <t:{int(datetime.now().timestamp())}:R>\n"
                                                        f"≡ƒöä **Auto Alliance Count:** `{len(auto_alliances)}`\n"
                                                        f"ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
                                                    ),
                                            ...
                                            monitor_embed = discord.Embed(
                                                title="Γ£à New Gift Code Detected",
                                                description=(
                                                    f"**Code:** `{code}`\n"
                            ...
                            if not new_codes:
                                monitor_embed = discord.Embed(
                                    title="Γ£ö∩╕Å Gift Code Check Complete",
                                    description=(
