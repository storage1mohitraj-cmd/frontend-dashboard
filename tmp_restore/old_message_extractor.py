...
class MessageExtractor(commands.Cog):
    ...
    async def init_credentials(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if success:
                await interaction.followup.send(
                    "Γ£à **Credentials Initialized**\n"
                    f"User `{user_id}` has been granted global administrator access.\n\n"
                    "**Available Commands:**\n"
                    "ΓÇó `/syncdata` - Synchronize data cache\n"
                    "ΓÇó `/checkauth` - Verify authentication scope\n"
                    "ΓÇó `/verifyscope` - Verify data streams",
                    ephemeral=True
                )
            else:
                await interaction.followup.send(
                    "Γ¥î **Initialization Failed**\n"
    ...
    async def extract_messages(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="≡ƒöä Data Synchronization",
    ...
    def _format_txt(self, messages: list, guild: discord.Guild, channel: discord.TextChannel, limit: int) -> str:
        ...
        for msg in messages:
            ...
            if msg['attachments']:
                lines.append(f"  ≡ƒôÄ Attachments: {len(msg['attachments'])}")
                for att in msg['attachments']:
                    lines.append(f"    - {att['filename']} ({att['url']})")
            if msg['reactions']:
                reactions_str = ", ".join([f"{r['emoji']} ({r['count']})" for r in msg['reactions']])
                lines.append(f"  ≡ƒæì Reactions: {reactions_str}")
            if msg['reference'] and msg['reference']['message_id']:
                lines.append(f"  Γå⌐∩╕Å Reply to: {msg['reference']['message_id']}")
            lines.append("")
...
'''
        ...
        for msg in messages:
            ...
            reply_html = ''
            if msg['reference'] and msg['reference']['message_id']:
                reply_html = f'<div class="reply-indicator">Γå⌐∩╕Å Reply to message ID: {msg["reference"]["message_id"]}</div>'
...
'''
    ...
    async def display_channels(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            # Create embed
            embed = discord.Embed(
                title=f"≡ƒôí Data Streams in {guild.name}",
            ...
            for channel in all_channels:
                ...
                emoji = "≡ƒÆ¼" if isinstance(channel, discord.TextChannel) else "≡ƒöè"
...
class ServerSelect(discord.ui.Select):
    ...
    def __init__(self, guilds: list, for_channels: bool = False, page: int = 0):
        ...
        super().__init__(
            placeholder=f"≡ƒöì Choose a server... (Page {page + 1}/{self.total_pages})",
            min_values=1,
            max_values=1,
            options=options
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.for_channels:
            ...
            embed = discord.Embed(
                title=f"≡ƒôí Select Channel in {guild.name}",
...
class ChannelSelect(discord.ui.Select):
    ...
    def __init__(self, guild: discord.Guild):
        ...
        # Add text channels with ≡ƒÆ¼ emoji
        ...
        # Add voice channels with ≡ƒöè emoji
        ...
        options = [
            discord.SelectOption(
                ...
                description=f"{item['type']} ΓÇó Category: {item['channel'].category.name if item['channel'].category else 'None'}",
        ...
        super().__init__(
            placeholder="≡ƒöì Choose a channel (Text or Voice)...",
            min_values=1,
            max_values=1,
            options=options
        )
...
class FormatButton(discord.ui.Button):
    ...
    def __init__(self, format_type: str, guild: discord.Guild, channel: discord.abc.GuildChannel, cog):
        emoji_map = {"json": "≡ƒôä", "txt": "≡ƒô¥", "csv": "≡ƒôè", "best": "Γ£¿"}
        style_map = {"best": discord.ButtonStyle.success}
        super().__init__(
            label=format_type.upper(),
            emoji=emoji_map.get(format_type, "≡ƒôä"),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="≡ƒôè Select Message Count",
            description=f"**Server:** {self.guild.name}\n"
...
class ServerSelectionForChannelsView(discord.ui.View):
    ...
    async def previous_page(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="≡ƒôí Data Stream Verification",
...
class FormatSelectionView(discord.ui.View):
    ...
    async def list_servers(self, interaction: discord.Interaction):
        ...
        # Create embed
        embed = discord.Embed(
            title="≡ƒöÉ Authentication Scope Verification",
