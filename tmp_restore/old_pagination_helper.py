...
class ResultsPaginationView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.author_id:
            await interaction.response.send_message(
                "Γ¥î You cannot use these buttons.",
                ephemeral=True
            )
            return False
        return True
    ...
    @discord.ui.button(emoji="Γ¼à∩╕Å", style=discord.ButtonStyle.secondary, custom_id="prev_page")
    ...
    @discord.ui.button(emoji="Γ₧í∩╕Å", style=discord.ButtonStyle.secondary, custom_id="next_page")
...
def create_alliance_log_embeds(
    ...
    if not ids_list:
        ...
        embed = discord.Embed(
            title="≡ƒæÑ Members Added to Alliance",
            description=(
                f"**Alliance:** {alliance_name}\n"
                f"**Administrator:** {admin_name} (`{admin_id}`)\n"
                f"**Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                f"**Results:**\n"
                f"Γ£à Successfully Added: {added_count}\n"
                f"Γ¥î Failed: {error_count}\n"
                f"ΓÜá∩╕Å Already Exists: {already_exists_count}\n\n"
                "**Added FIDs:**\n"
                "```\nNo FIDs to display\n```"
            ),
