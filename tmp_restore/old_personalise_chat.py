...
class PronounSelectView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="≡ƒæñ Step 1: Your Pronouns - How should the bot refer to you?",
        options=[
            discord.SelectOption(label="He/Him", value="he/him", emoji="≡ƒæ¿"),
            discord.SelectOption(label="She/Her", value="she/her", emoji="≡ƒæ⌐"),
            discord.SelectOption(label="They/Them", value="they/them", emoji="≡ƒºæ"),
            discord.SelectOption(label="Other", value="other", emoji="Γ£¿"),
        ]
    )
    async def pronoun_select(self, interaction: discord.Interaction, select: discord.ui.Select):
        if str(interaction.user.id) != self.user_id:
            await interaction.response.send_message("Γ¥î This is not your personalization session.", ephemeral=True)
            return
...
class TraitSelectView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="Γ£¿ Select up to 3 personality traits",
        options=[
...
class PlayerIDModal(discord.ui.Modal, title="≡ƒÄ« Step 3: Player Information"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not re.fullmatch(r"\d{9}", player_id):
            await interaction.response.send_message(
                "Γ¥î Invalid player ID. Must be exactly 9 digits.",
                ephemeral=True
            )
            return
        ...
        try:
            ...
            if not player_data:
                await interaction.followup.send(
                    "Γ¥î Could not validate player ID. Please check your player ID and try again.",
                    ephemeral=True
                )
                return
            ...
            embed = discord.Embed(
                title="Γ£à Personalization Complete!",
                description=personalized_response,
                color=0x2ecc71
            )
            embed.add_field(name="≡ƒæñ Pronouns", value=f"`{self.pronouns}`", inline=True)
            embed.add_field(name="Γ£¿ Traits", value=f"`{', '.join(self.traits)}`", inline=True)
            embed.add_field(name="≡ƒÄ« Player ID", value=f"`{player_id}`", inline=True)
            embed.add_field(name="≡ƒôè Current Stats", value=f"**{player_name}** ΓÇó FC {furnace_level} ΓÇó State {state_id}", inline=False)
            ...
            await interaction.followup.send(
                f"Γ¥î An error occurred during personalization: {str(e)}",
                ephemeral=True
            )
...
Keep it friendly, enthusiastic, and tailored to their traits!"""
            ...
            if response and response.strip():
                ...
                return f"Welcome, {player_name}! ≡ƒÄë I've saved your preferences and I'm excited to chat with you. With a furnace level of {furnace_level}, you're doing great! Let's make our conversations awesome together! ≡ƒÜÇ"
        
        except Exception as e:
            logger.error(f"Error generating personalized response: {e}")
            # Fallback message
            return f"Welcome, {player_name}! ≡ƒÄë Your preferences have been saved. I'm here to help with all your Whiteout Survival questions!"
...
class PersonaliseChatCog(commands.Cog):
    ...
    async def personalisechat(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed = discord.Embed(
                title="≡ƒÄ¿ Personalize Your Chat Experience",
                description=(
                    "Let's make our conversations more personal! ≡ƒîƒ\n\n"
                    "I'll ask you a few quick questions to understand you better:\n"
                    "1∩╕ÅΓâú **Your Pronouns** - How should I refer to you?\n"
                    "2∩╕ÅΓâú **Your Personality** - What traits describe you?\n"
                    "3∩╕ÅΓâú **Your Game Info** - Your player details\n\n"
                    "This helps me tailor responses just for you! ≡ƒÆ¼"
                ),
                color=0x3498db
            )
            ...
            if interaction.response.is_done():
                await interaction.followup.send(
                    "Γ¥î An error occurred while starting personalization. Please try again.",
                    ephemeral=True
                )
            else:
                await interaction.response.send_message(
