...
class RemoteAccess(commands.Cog):
            ...
            try:
                ...
                if (not result or result[0] != 1) and not await is_bot_owner(self.bot, interaction.user.id):
                    await interaction.response.send_message(
                        "Γ¥î Only global administrators can use Remote Access.",
                        ephemeral=True
                    )
                    return
                ...
                if not guilds:
                    await interaction.response.send_message(
                        "Γ¥î Bot is not in any servers.",
                        ephemeral=True
                    )
                    return

                embed = discord.Embed(
                    title="≡ƒîÉ Remote Access Control Panel",
                    description=(
                        "**Manage Channels Across All Servers**\n"
                        "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n\n"
                        f"≡ƒôè **Connected Servers:** `{len(guilds)}`\n"
                        f"≡ƒæÑ **Total Members:** `{sum(g.member_count for g in guilds)}`\n\n"
                        "**Available Actions:**\n"
                        "ΓÇó ≡ƒôï View server channels\n"
                        "ΓÇó Γ₧ò Create new channels\n"
                        "ΓÇó Γ£Å∩╕Å Edit existing channels\n"
                        "ΓÇó ≡ƒùæ∩╕Å Delete channels\n"
                        "ΓÇó ≡ƒô¿ Send messages to any channel\n"
                        "ΓÇó ≡ƒÄ╡ Play music remotely\n"
                        "ΓÇó ≡ƒ¢í∩╕Å Start alliance monitoring\n"
                        "ΓÇó ≡ƒöÆ Manage permissions\n\n"
                        "ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
                ...
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        "Γ¥î An error occurred while loading Remote Access.",
                        ephemeral=True
                    )
    ...
    async def show_server_management(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"≡ƒÅ░ Managing: {guild.name}",
                description=(
                    f"**Server Information**\n"
                    f"ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n\n"
                    f"≡ƒæÑ **Members:** `{guild.member_count}`\n"
                    f"≡ƒô¥ **Text Channels:** `{len(text_channels)}`\n"
                    f"≡ƒöè **Voice Channels:** `{len(voice_channels)}`\n"
                    f"≡ƒôü **Categories:** `{len(categories)}`\n\n"
                    f"≡ƒöæ **Bot Permissions:**\n"
                    f"{'Γ£à' if has_manage_channels else 'Γ¥î'} Manage Channels\n"
                    f"{'Γ£à' if bot_member.guild_permissions.administrator else 'Γ¥î'} Administrator\n\n"
                    f"ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü\n"
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while loading server management.",
                ephemeral=True
            )
    ...
    async def view_channels(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            # Build embed
            embed = discord.Embed(
                title=f"≡ƒôï Channels in {guild.name}",
            ...
            if uncategorized_channels:
                ...
                for channel in uncategorized_channels[:10]:  # Limit to prevent embed size issues
                    emoji = "≡ƒô¥" if isinstance(channel, discord.TextChannel) else "≡ƒöè" if isinstance(channel, discord.VoiceChannel) else "≡ƒôó"
                    channel_list += f"{emoji} {channel.mention if isinstance(channel, discord.TextChannel) else channel.name}\n"
                
                if channel_list:
                    embed.add_field(
                        name="≡ƒôî Uncategorized",
                        value=channel_list,
                        inline=False
                    )
            ...
            await interaction.followup.send(
                "Γ¥î An error occurred while viewing channels.",
                ephemeral=True
            )
    ...
    async def create_channel(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
                    ...
                    try:
                        ...
                        if channel_type_str not in ['text', 'voice', 'category']:
                            await modal_interaction.response.send_message(
                                "Γ¥î Invalid channel type. Use: text, voice, or category",
                                ephemeral=True
                            )
                            return
                        ...
                        if created_channel:
                            ...
                            await modal_interaction.followup.send(
                                "Γ¥î Failed to create channel.",
                                ephemeral=True
                            )
                    
                    except discord.Forbidden:
                        await modal_interaction.followup.send(
                            "Γ¥î I don't have permission to create channels in this server.",
                            ephemeral=True
                        )
                    except Exception as e:
                        print(f"Create channel modal error: {e}")
                        await modal_interaction.followup.send(
                            f"Γ¥î An error occurred: {str(e)}",
                            ephemeral=True
                        )
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while creating the channel.",
                ephemeral=True
            )
    ...
    async def edit_channel(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not channels:
                await interaction.response.send_message(
                    "Γ¥î No editable channels found in this server.",
                    ephemeral=True
                )
                return
            ...
            options = [
                discord.SelectOption(
                    ...
                    description=f"{'Text' if isinstance(channel, discord.TextChannel) else 'Voice'} ΓÇó Category: {channel.category.name if channel.category else 'None'}",
                    emoji="≡ƒô¥" if isinstance(channel, discord.TextChannel) else "≡ƒöè"
                )
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            if self.new_name.value and self.new_name.value != channel.name:
                                ...
                                changes.append(f"**Name:** `{channel.name}` ΓåÆ `{self.new_name.value}`")
                            ...
                            if changes:
                                ...
                                await modal_int.followup.send(
                                    "Γä╣∩╕Å No changes made.",
                                    ephemeral=True
                                )
                        
                        except discord.Forbidden:
                            await modal_int.followup.send(
                                "Γ¥î I don't have permission to edit this channel.",
                                ephemeral=True
                            )
                        except Exception as e:
            ...
            embed = discord.Embed(
                title=f"Γ£Å∩╕Å Edit Channel in {guild.name}",
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while editing the channel.",
                ephemeral=True
            )
    ...
    async def delete_channel(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not channels:
                await interaction.response.send_message(
                    "Γ¥î No channels found in this server.",
                    ephemeral=True
                )
                return
            ...
            options = [
                discord.SelectOption(
                    ...
                    emoji="≡ƒô¥" if isinstance(channel, discord.TextChannel) else "≡ƒöè" if isinstance(channel, discord.VoiceChannel) else "≡ƒôü"
                )
                for channel in sorted(channels, key=lambda c: c.position)[:25]
            ]
            ...
            async def channel_selected(select_interaction: discord.Interaction):
                ...
                confirm_embed = discord.Embed(
                    title="ΓÜá∩╕Å Confirm Channel Deletion",
                    description=(
                        ...
                        f"**Server:** {guild.name}\n\n"
                        f"ΓÜá∩╕Å **This action cannot be undone!**"
                    ),
                    color=0xED4245
                )
                ...
                async def confirm_delete(confirm_int: discord.Interaction):
                    try:
                        ...
                        await confirm_int.response.send_message(
                            "Γ¥î I don't have permission to delete this channel.",
                            ephemeral=True
                        )
                    except Exception as e:
            ...
            embed = discord.Embed(
                title=f"≡ƒùæ∩╕Å Delete Channel in {guild.name}",
                description="ΓÜá∩╕Å Select a channel to delete. **This action cannot be undone!**",
                color=0xED4245
            )
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while preparing channel deletion.",
                ephemeral=True
            )
    ...
    async def send_message(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not channels:
                await interaction.response.send_message(
                    "Γ¥î No accessible text channels found in this server where I can send messages.",
                    ephemeral=True
                )
                return
            ...
            options = [
                discord.SelectOption(
                    label=f"{channel.name[:90]}",
                    value=str(channel.id),
                    description=f"Category: {channel.category.name if channel.category else 'None'} ΓÇó {channel.topic[:50] if channel.topic else 'No topic'}",
                    emoji="≡ƒô¥"
                )
                for channel in sorted(channels, key=lambda c: c.position)[:25]
            ]
            ...
            embed = discord.Embed(
                title=f"≡ƒô¿ Send Message in {guild.name}",
                description=(
                    "**Select a channel to send a message**\n\n"
                    "You'll be able to:\n"
                    "ΓÇó Send plain text messages\n"
                    "ΓÇó Create rich embeds\n"
                    "ΓÇó Send messages with custom formatting\n\n"
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while preparing to send message.",
                ephemeral=True
            )
    ...
    async def show_message_type_selection(self, interaction: discord.Interaction, channel: discord.TextChannel, guild: discord.Guild):
        ...
        try:
                ...
                class PlainMessageModal(Modal, title=f"Send to #{channel.name}"):
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            await modal_int.followup.send(
                                "Γ¥î I don't have permission to send messages in this channel.",
                                ephemeral=True
                            )
                        except Exception as e:
                            print(f"Send message error: {e}")
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            announcement_embed = discord.Embed(
                                title=f"≡ƒôó {self.announcement_title.value}",
                            ...
                            try:
                                ...
                                await modal_int.followup.send(
                                    "Γ¥î I don't have permission to send announcements or mention everyone/here.",
                                    ephemeral=True
                                )
            ...
            # Back button
            back_button = discord.ui.Button(
                label="ΓùÇ Back to Channel Selection",
                style=discord.ButtonStyle.secondary
            )
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while loading message options.",
                ephemeral=True
            )
    ...
    async def play_music(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not music_cog:
                await interaction.response.send_message(
                    "Γ¥î Music system is not loaded.",
                    ephemeral=True
                )
                return
            ...
            if not voice_channels:
                await interaction.response.send_message(
                    "Γ¥î No voice channels found in this server.",
                    ephemeral=True
                )
                return
            ...
            if not accessible_channels:
                await interaction.response.send_message(
                    "Γ¥î Bot doesn't have permission to connect or speak in any voice channels.",
                    ephemeral=True
                )
                return
            ...
            options = [
                discord.SelectOption(
                    label=f"{vc.name[:90]}",
                    value=str(vc.id),
                    description=f"Members: {len(vc.members)} ΓÇó Category: {vc.category.name if vc.category else 'None'}",
                    emoji="≡ƒöè"
                )
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            try:
                                ...
                                if not player:
                                    await modal_int.followup.send(
                                        "Γ¥î Failed to establish voice connection.",
                                        ephemeral=True
                                    )
                                    return
                                ...
                                if not music_cog:
                                    await modal_int.followup.send(
                                        "Γ¥î Music system not available.",
                                        ephemeral=True
                                    )
                                    return
                                ...
                                try:
                                    ...
                                    if not tracks:
                                        await modal_int.followup.send(
                                            f"Γ¥î No results found for: `{query}`",
                                            ephemeral=True
                                        )
                                        return
                                    ...
                                    success_embed = discord.Embed(
                                        title="≡ƒÄ╡ Music Playing Remotely",
                                        description=(
                                            f"**Song:** {track.title}\n"
                                            ...
                                            f"{'Γû╢∩╕Å Now Playing' if player.playing and not player.queue.is_empty else '≡ƒôï Added to Queue'}"
                                        ),
                                        color=0x57F287
                                    )
                                    ...
                                    traceback.print_exc()
                                    await modal_int.followup.send(
                                        f"Γ¥î Error playing music: {str(play_error)}",
                                        ephemeral=True
                                    )
                            
                            except discord.Forbidden:
                                await modal_int.followup.send(
                                    "Γ¥î Bot doesn't have permission to connect to this voice channel.",
                                    ephemeral=True
                                )
                            except Exception as connect_error:
                                print(f"Voice connect error: {connect_error}")
                                await modal_int.followup.send(
                                    f"Γ¥î Error connecting to voice channel: {str(connect_error)}",
                                    ephemeral=True
                                )
            ...
            embed = discord.Embed(
                title=f"≡ƒÄ╡ Play Music in {guild.name}",
                description=(
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while setting up music playback.",
                ephemeral=True
            )
    ...
    async def start_alliance_monitor(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not alliance_cog:
                await interaction.response.send_message(
                    "Γ¥î Alliance system is not loaded.",
                    ephemeral=True
                )
                return
            ...
            try:
                ...
                await interaction.response.send_message(
                    "Γ¥î MongoDB not enabled. Alliance monitoring requires MongoDB.",
                    ephemeral=True
                )
                return
            ...
            async def alliance_selected(select_interaction: discord.Interaction):
                ...
                if not text_channels:
                    await select_interaction.response.send_message(
                        "Γ¥î No accessible text channels found in this server.",
                        ephemeral=True
                    )
                    return
                ...
                async def channel_selected(channel_interaction: discord.Interaction):
                    ...
                    try:
                        ...
                        # Create success embed
                        success_embed = discord.Embed(
                            title="Γ£à Alliance Monitoring Started",
                            description=(
                                ...
                                f"**Monitoring Active** Γ£à\n"
                                f"The system will check for changes every 4 minutes.\n\n"
                                f"**Tracked Changes:**\n"
                                f"ΓÇó ≡ƒæñ Name changes\n"
                                f"ΓÇó ≡ƒöÑ Furnace level changes\n"
                                f"ΓÇó ≡ƒû╝∩╕Å Avatar changes"
                            ),
                            color=0x57F287
                        )
                        ...
                        try:
                            channel_embed = discord.Embed(
                                title="≡ƒ¢í∩╕Å Alliance Monitoring Started",
                                description=(
                        ...
                        await channel_interaction.followup.send(
                            f"Γ¥î Error saving monitoring configuration: {str(save_error)}",
                            ephemeral=True
                        )
                ...
                channel_embed = discord.Embed(
                    title=f"≡ƒ¢í∩╕Å Monitor: {alliance_name}",
                    description=(
                        f"**Selected Alliance:** {alliance_name}\n"
                        f"**Alliance ID:** `{selected_alliance_id}`\n\n"
                        "Now select the channel where monitoring updates should be posted.\n\n"
                        "**What will be monitored:**\n"
                        "ΓÇó ≡ƒæñ Player name changes\n"
                        "ΓÇó ≡ƒöÑ Furnace level changes\n"
                        "ΓÇó ≡ƒû╝∩╕Å Avatar changes\n\n"
                        "Select a channel from the dropdown below."
                    ),
                    color=0x5865F2
                )
            ...
            embed = discord.Embed(
                title=f"≡ƒ¢í∩╕Å Alliance Monitor for {guild.name}",
                description=(
                    ...
                    "**Features:**\n"
                    "ΓÇó Monitor alliance member changes\n"
                    "ΓÇó Track name, furnace, and avatar changes\n"
                    "ΓÇó Automatic updates every 4 minutes\n"
                    "ΓÇó Real-time notifications\n\n"
            ...
            if current_alliance:
                embed.add_field(
                    name="≡ƒôî Current Server Alliance",
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while setting up alliance monitoring.",
                ephemeral=True
            )
    ...
    async def stop_alliance_monitor(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            try:
                ...
                await interaction.response.send_message(
                    "Γ¥î Database utilities not available.",
                    ephemeral=True
                )
                return
            ...
            for alliance_id, channel_id, enabled in active_monitors:
                ...
                monitor_options.append(
                    discord.SelectOption(
                        ...
                        description=f"Channel: #{channel_name[:40]} ΓÇó ID: {alliance_id}",
                        emoji="≡ƒ¢í∩╕Å"
                    )
                )
            ...
            if not monitor_options:
                await interaction.response.send_message(
                    "Γ¥î No valid monitors found to stop.",
                    ephemeral=True
                )
                return
            ...
            async def monitor_selected(select_interaction: discord.Interaction):
                ...
                async def confirm_stop(button_int: discord.Interaction):
                    try:
                        ...
                        # Create success embed
                        success_embed = discord.Embed(
                            title="Γ£à Alliance Monitoring Stopped",
                            description=(
                                f"**Alliance:** {alliance_name}\n"
                                f"**Alliance ID:** `{alliance_id}`\n"
                                f"**Channel:** {channel_mention}\n"
                                f"**Server:** {guild.name}\n\n"
                                f"**Status:** Γ¥î Monitoring Disabled\n\n"
                        ...
                        if channel:
                            try:
                                channel_embed = discord.Embed(
                                    title="≡ƒ¢æ Alliance Monitoring Stopped",
                                    description=(
                        ...
                        await button_int.followup.send(
                            f"Γ¥î Error stopping monitor: {str(stop_error)}",
                            ephemeral=True
                        )
                
                async def cancel_stop(button_int: discord.Interaction):
                    cancel_embed = discord.Embed(
                        title="Γ¥î Action Cancelled",
                ...
                confirm_embed = discord.Embed(
                    title="ΓÜá∩╕Å Confirm Stop Monitoring",
                    description=(
            ...
            embed = discord.Embed(
                title=f"≡ƒ¢æ Stop Alliance Monitor for {guild.name}",
                description=(
                    f"**Active Monitors:** {len(active_monitors)}\n\n"
                    "Select an alliance monitor to stop from the dropdown below.\n\n"
                    "**What happens when you stop:**\n"
                    "ΓÇó Monitoring will be disabled\n"
                    "ΓÇó No more change notifications will be sent\n"
                    "ΓÇó You can restart monitoring anytime\n\n"
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while loading the stop monitor menu.",
                ephemeral=True
            )
    ...
    async def kick_user(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not bot_member or not bot_member.guild_permissions.kick_members:
                await interaction.response.send_message(
                    "Γ¥î I don't have **Kick Members** permission in this server.",
                    ephemeral=True
                )
                return
            ...
            if not members:
                await interaction.response.send_message(
                    "Γ¥î No kickable members found in this server.\n"
            ...
            options = [
                discord.SelectOption(
                    ...
                    value=str(m.id),
                    description=f"@{str(m)[:40]} ΓÇó Joined: {m.joined_at.strftime('%Y-%m-%d') if m.joined_at else 'Unknown'}",
                    emoji="≡ƒæñ"
                )
            ...
            async def member_selected(select_interaction: discord.Interaction):
                ...
                if not target:
                    await select_interaction.response.send_message(
                        "Γ¥î Member not found. They may have already left the server.",
                        ephemeral=True
                    )
                    return
                ...
                class KickReasonModal(Modal, title=f"Kick {target.display_name}"):
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            confirm_embed = discord.Embed(
                                title="ΓÜá∩╕Å Confirm Member Kick",
                                description=(
                                    ...
                                    f"ΓÜá∩╕Å **This will remove the member from the server.**\n"
                            ...
                            async def confirm_kick(confirm_int: discord.Interaction):
                                try:
                                    ...
                                    await confirm_int.response.edit_message(embed=success_embed, view=None)
                                    print(f"[REMOTE ACCESS] Kicked {member_name} from {self.guild.name} ΓÇö Reason: {kick_reason} | By: {confirm_int.user}")

                                except discord.Forbidden:
                                    await confirm_int.response.send_message(
                                        "Γ¥î I don't have permission to kick this member.\n"
                                        "Their role may be equal to or higher than mine.",
                                        ephemeral=True
                                    )
                                except discord.HTTPException as http_err:
                                    await confirm_int.response.send_message(
                                        f"Γ¥î Discord API error: {str(http_err)}",
                                        ephemeral=True
                                    )
                                except Exception as kick_err:
                                    print(f"Kick error: {kick_err}")
                                    await confirm_int.response.send_message(
                                        f"Γ¥î An error occurred: {str(kick_err)}",
                                        ephemeral=True
                                    )

                            async def cancel_kick(cancel_int: discord.Interaction):
                                cancel_embed = discord.Embed(
                                    title="Γ¥î Kick Cancelled",
            ...
            shown = min(total, 25)
            embed = discord.Embed(
                title=f"≡ƒæó Kick Member from {guild.name}",
                description=(
                    f"**Server Members:** `{guild.member_count}`\n"
                    f"**Kickable Members:** `{total}` (showing first `{shown}`)\n\n"
                    "**Kick Flow:**\n"
                    "1∩╕ÅΓâú Select a member from the dropdown\n"
                    "2∩╕ÅΓâú Enter an optional kick reason\n"
                    "3∩╕ÅΓâú Confirm to execute the kick\n\n"
                    "ΓÜá∩╕Å Only members whose highest role is **below** the bot's role are shown.\n"
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while loading the kick menu.",
                ephemeral=True
            )
    ...
    async def delete_message_by_id(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        try:
            ...
            if not channels:
                await interaction.response.send_message(
                    "Γ¥î I don't have **Manage Messages** permission in any text channel of this server.",
                    ephemeral=True
                )
                return

            # Step 1 ΓÇô channel dropdown
            options = [
                discord.SelectOption(
                    label=f"{channel.name[:90]}",
                    value=str(channel.id),
                    description=f"Category: {channel.category.name if channel.category else 'None'} ΓÇó {channel.topic[:40] if channel.topic else 'No topic'}",
                    emoji="≡ƒô¥"
                )
                for channel in sorted(channels, key=lambda c: c.position)[:25]
            ]
            ...
            async def channel_selected(select_interaction: discord.Interaction):
                ...
                # Step 2 ΓÇô modal to enter the message ID
                from discord.ui import Modal, TextInput
                    ...
                    async def on_submit(self, modal_int: discord.Interaction):
                        try:
                            ...
                            if not raw_id.isdigit():
                                await modal_int.response.send_message(
                                    "Γ¥î Invalid message ID. Please enter a numeric Discord message ID.",
                                    ephemeral=True
                                )
                                return
                            ...
                            try:
                                ...
                                await modal_int.response.send_message(
                                    f"Γ¥î Message `{msg_id}` was not found in {self.channel.mention}.\n"
                                ...
                                await modal_int.response.send_message(
                                    "Γ¥î I don't have permission to read messages in that channel.",
                                    ephemeral=True
                                )
                                return
                            ...
                            # Step 3 ΓÇô confirmation
                            confirm_embed = discord.Embed(
                                title="ΓÜá∩╕Å Confirm Message Deletion",
                                description=(
                            ...
                            async def confirm_delete(confirm_int: discord.Interaction):
                                try:
                                    ...
                                    await confirm_int.response.send_message(
                                        "Γ¥î I don't have permission to delete that message.",
                                        ephemeral=True
                                    )
                                except discord.NotFound:
                                    await confirm_int.response.send_message(
                                        "Γ¥î The message was already deleted or could not be found.",
                                        ephemeral=True
                                    )
                                except Exception as del_err:
                                    print(f"Delete message error: {del_err}")
                                    await confirm_int.response.send_message(
                                        f"Γ¥î An error occurred while deleting: {str(del_err)}",
                                        ephemeral=True
                                    )
            ...
            embed = discord.Embed(
                title=f"≡ƒùæ∩╕Å Delete Message in {guild.name}",
                description=(
                    "**Delete a Specific Message by ID**\n\n"
                    "**How to get a Message ID:**\n"
                    "1. Enable **Developer Mode** in Discord settings\n"
                    "2. Right-click any message ΓåÆ **Copy Message ID**\n\n"
                    "**Steps:**\n"
                    "1∩╕ÅΓâú Select the channel that contains the message\n"
                    "2∩╕ÅΓâú Enter the Message ID\n"
                    "3∩╕ÅΓâú Confirm the deletion\n\n"
                    "ΓÜá∩╕Å **This action is permanent and cannot be undone.**"
                ),
                color=0xED4245
            )
            ...
            traceback.print_exc()
            await interaction.response.send_message(
                "Γ¥î An error occurred while setting up message deletion.",
                ephemeral=True
            )
    ...
    async def manage_permissions(self, interaction: discord.Interaction, guild: discord.Guild):
        ...
        await interaction.response.send_message(
            "≡ƒöÆ **Manage Permissions**\n\nThis feature is coming soon! It will allow you to:\n"
            "ΓÇó View channel permissions\n"
            "ΓÇó Modify role permissions\n"
            "ΓÇó Set user-specific overrides\n"
            "ΓÇó Copy permissions between channels",
            ephemeral=True
        )
